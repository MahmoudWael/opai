...
export interface TicketProvider { readonly identity: string; listAssigned(): Promise<Ticket[]>; listSavedQueries(): Promise<SavedQuery[]>; listQueryTickets(queryId: string): Promise<Ticket[]>; get(id: string): Promise<Ticket>; promptTemplate(kind: PromptKind): string; prompt(ticket: Ticket, action: TicketAction, template?: string): string }
